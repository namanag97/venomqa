...
def demo(port: int, server_only: bool, keep_running: bool, verbose: bool, explain: bool) -> None:
    ...
    if explain:
        console.print("[bold]Example Journey Code:[/bold]")
        console.print()
        console.print("[dim]```python[/dim]")
        console.print("[yellow]from venomqa import Journey, Step, Client[/yellow]")
        console.print("[yellow]from venomqa.runner import JourneyRunner[/yellow]")
        console.print()
        console.print("[green]def login(client, ctx):[/green]")
        console.print("[green]    response = client.post('/auth/login', json={...})[/green]")
        console.print("[green]    ctx['token'] = response.json()['token'][/green]")
        console.print("[green]    return response[/green]")
        console.print()
        console.print("[cyan]journey = Journey(name='my_flow', steps=[[/cyan]")
        console.print("[cyan]    Step(name='login', action=login),[/cyan]")
